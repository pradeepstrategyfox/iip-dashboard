...
async function fetchCSV(id, sheetName) {
  ...
  if (errors.length > 0) {
    console.warn(`CSV parse warnings for ${sheetName || 'default'}:`, errors.slice(0, 3));
  }
  return data;
}
...
// ─── Process raw Meta leads (New + Old campaigns) ──────────────────────────────
function processMetaLeads(rows, campaign) {
  return rows
    ...
    .map((r) => ({
      ...
      industry: (
        r['which_industry_to_you_currently_work_in?'] || ''
      )
        .trim()
        .toLowerCase(),
      interest: (
        r['why_are_you_interested_in_this_program?'] || ''
      )
        .trim()
        .toLowerCase()
        .replace(/_/g, ' '),
...
// ─── Process IIP Leads Sheet1 (Old campaign - support team data) ────────────────
function processSheet1(rows) {
  return rows
    ...
    .map((r) => ({
      ...
      otherDetails: (r['OTHER DETAILS'] || '').trim(),
      followUp: (r['FOLLOW UP UPDATES'] || '').trim(),
      iipFollowup: (r['IIP FOLLOWUP'] || r['IIP FOLLOWUP '] || '').trim(),
      team: (r['IIP Team'] || '').trim(),
    }));
}

// ─── Process IIP Leads Hindi Leads (New campaign - support team data) ──────────
function processHindiLeads(rows) {
  return rows
    .filter((r) => r.NAME)
    .map((r) => {
      // The industry column header is a space " " in this sheet
      const industry =
        r[''] || r[' '] || r['  '] || Object.values(r)[4] || '';
      return {
...
function aggregate(metaNew, metaOld, sheet1, hindiLeads) {
  ...
  const callRate =
    totalSupportLeads > 0
      ? ((totalCallsMade / totalSupportLeads) * 100).toFixed(1)
      : '0';
  const responseRate =
    totalCallsMade > 0
      ? ((totalResponded / totalCallsMade) * 100).toFixed(1)
      : '0';

  // === Daily Leads (from Meta raw data) ===
  const dailyMap = {};
  for (const lead of allMetaLeads) {
    if (!lead.date) continue;
    if (!dailyMap[lead.date]) {
      dailyMap[lead.date] = { new: 0, old: 0 };
    }
  ...
  // === Status Distribution (from support team sheets) ===
  const statusMap = {};
  ...
  const callPerf = {
    old: {
      total: sheet1.length,
      called: sheet1.filter((l) => l.called).length,
      responded: sheet1.filter((l) => l.responded).length,
    },
    new: {
      total: hindiLeads.length,
      called: hindiLeads.filter((l) => l.called).length,
      responded: hindiLeads.filter((l) => l.responded).length,
    },
  };

  // === Team Performance (from Sheet1 which has IIP Team column) ===
  const teamMap = {};
  ...
  // === Recent Leads (last 20 from meta) ===
...
async function fetchAllData(forceRefresh = false) {
  ...
  console.log(
    `[${new Date().toISOString()}] Data fetched. Meta leads: ${metaNew.length + metaOld.length}, Support leads: ${sheet1.length + hindiLeads.length}`
  );
  return data;
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'ok',
    cached: !!cache.data,
    cacheAge: cache.data ? Math.round((Date.now() - cache.timestamp) / 1000) + 's' : null,
  });
});
...
app.listen(PORT, () => {
  ...
  // Pre-warm cache
  fetchAllData().catch((err) => console.error('Initial fetch error:', err));
});
